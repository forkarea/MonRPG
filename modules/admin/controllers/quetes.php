...
class Quetes_Controller extends Template_Controller {
		...
		public function show( $idQuete = false )
		{
				...
				if( !$module = Map_Model::instance()->select_detail( array( 'module_map' => 'quete' ), false ) )
						return url::redirect( 'regions?msg='.urlencode( Kohana::lang( 'quete.no_module' ) ) );

				$this->script = array( 'jquery.validate', 'jquery.facebox', 'quetes' );
				...
				$this->template->contenu->formulaire->articles = Article_Model::instance()->select( array( 'article_category_id' => 3, 'status' => 1 ) );
				$this->template->contenu->formulaire->module = $module;
				$this->template->contenu->formulaire->items = Item_Model::instance()->select();
				$this->template->contenu->formulaire->bots = Map_Model::instance()->select_detail( array( 'module_map' => 'fight' ), false );
		...
		public function save( $type = FALSE, $idQuete = FALSE )
		{
				if( ($save = $this->input->post() ) !== FALSE )
				{
						...
						if( $type == 'sauve' || $type == 'valid' )
						{
								...
								if( $save['status'] && ($validateConf = $this->quete->select( array( 'element_detail_id_start' => $save['element_detail_id_start'], 'id_quete !=' => $idQuete ), TRUE ) ) )
										$save['status'] = 0;

								elseif( $save['status'] && ($validateConf = $this->quete->select( array( 'element_detail_id_stop' => $save['element_detail_id_stop'], 'id_quete !=' => $idQuete ), TRUE ) ) )
										$save['status'] = 0;

								$this->quete->update( $save, $idQuete );

								if( $validateConf )
										return url::redirect( 'quetes/show/'.$idQuete.'?msg='.urlencode( Kohana::lang( 'quete.conflit' ) ) );
						}
						elseif( $type == 'trash' )
		...
		public function resultatAjax()
		{
				...
				if( ($module = Map_Model::instance()->select_detail( array( 'module_map' => 'quete' ), false ) ) !== FALSE )
						foreach( $module as $row )
								$showModule[$row->id_detail] = $row;
				...
				foreach( $arrayResultat as $row )
				{
						...
						$v[] = (isset( $showModule[$row->element_detail_id_start] ) ? $showModule[$row->element_detail_id_start]->nom_map : Kohana::lang( 'form.inconnu' ) );
						$v[] = (isset( $showModule[$row->element_detail_id_stop] ) ? $showModule[$row->element_detail_id_stop]->nom_map : Kohana::lang( 'form.inconnu' ));
						$v[] = $row->niveau;
