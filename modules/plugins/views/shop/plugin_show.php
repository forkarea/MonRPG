...
<?php if( $listItem ) : ?>
		<form id="formBuy">
				<table width="500" class="table_shop">
						<?php foreach( $listItem as $row ) : ?>
								<?php if( $row->niveau <= $user->niveau && !isset( $itemProtect[$row->id] ) ) : ?>
										<tr>
												<?php if( $admin ) : ?>
												...
												<td align="left" width="100" class="<?php echo $row->hp || $row->attaque ? 'vert' : 'rouge'; ?>">
														<?php
												if( $row->hp )
		...
		<div class="formButton">
				<span id="show_buy" style="display:none">
						<input type="hidden" id="argent_user" value="<?php echo $user->argent; ?>"/>
						<input type="button" id="buy_item" class="button button_vert" value="<?php echo Kohana::lang( 'shop.button_buy' ); ?>"/>
				</span>
